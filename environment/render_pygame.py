...
class PyGameRenderer:
    ...
    def __init__(self, grid_size: int = 16, cell_size: int = 48, fps: int = 10):
        """
        Initialize the PyGame renderer (full grid visible).
        ...
        # Calculate window size based on full grid (no viewport needed)
        ...
        # Create window (768×868 for 16×16 grid with 48px cells)
    ...
    def _update_camera(self, agent_pos: Tuple[int, int]):
        """
        No camera system needed - entire grid is visible.
        Kept for compatibility but does nothing.

        Args:
            agent_pos: (y, x) position of agent
        """
        # No camera system - entire grid visible
        pass

    def _render_game_view(self, env_state: Dict[str, Any]):
        """
        Render the entire 16×16 game grid (no viewport needed).
        Agent has GLOBAL vision and entire grid is displayed.
        ...
        # Render all tiles (entire grid visible)
        for grid_y in range(self.grid_size):
            for grid_x in range(self.grid_size):
                # Screen position (direct mapping, no viewport)
    ...
    def _render_entity(self, position: Tuple[int, int], sprite_name: str):
        """
        Render an entity at a given position (direct mapping, no viewport).
