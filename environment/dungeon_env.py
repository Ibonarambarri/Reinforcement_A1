...
class DungeonCrawlerEnv(gym.Env):
    """
    Simplified Gymnasium environment for tabular RL navigation.
    ...
    - 2 mobile enemies with random movement

    Attributes:
        grid_size (int): Size of the square grid (16)
        max_steps (int): Maximum steps per episode (300)
        action_space: Discrete(4) - movement actions only
        observation_space: Dict with global_view (16×16) only
    """
        ...
        if render_mode == 'pygame':
            try:
                ...
                self.pygame_renderer = PyGameRenderer(
                    grid_size=grid_size,
                    cell_size=48,  # 48px cells for 16×16 grid (768×868 window)
    ...
    def _place_random_walls(self):
        """
        No interior walls - this method does nothing now.
        Kept for compatibility but doesn't place any walls.
        """
        # No interior walls - empty grid interior
        pass
    ...
    def reset(
        ...
        No interior walls - only border walls, so door is always reachable.
        ...
        # Create empty grid with border walls only (no interior walls)
        self._create_empty_grid()

        # Reset state
        self._init_state()

        # Random spawn positions for agent and door (no need for pathfinding validation)
